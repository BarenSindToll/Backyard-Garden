...
// ── Environmental surroundings (forest west, river east) ───────────────────

// Top-down forest canopy — layered green blobs over a deep-green base.
const FOREST_BG =
  'radial-gradient(circle at 22% 24%, #74a64d 0 6px, transparent 7px),' +
  'radial-gradient(circle at 64% 16%, #4f7e31 0 7px, transparent 8px),' +
  'radial-gradient(circle at 82% 66%, #6c9e46 0 6px, transparent 7px),' +
  'radial-gradient(circle at 36% 74%, #426c28 0 7px, transparent 8px),' +
  'radial-gradient(circle at 8% 52%, #5c8a3a 0 5px, transparent 6px),' +
  'linear-gradient(180deg, #3d6627 0%, #335821 100%)';
const FOREST_BG_SIZE = '29px 29px, 35px 35px, 31px 31px, 33px 33px, 27px 27px, 100% 100%';

const PLOT_GROUND =
  'radial-gradient(circle at 30% 18%, rgba(255,255,255,0.35) 0 1px, transparent 2px),' +
  'radial-gradient(circle at 72% 60%, rgba(150,120,70,0.18) 0 1.5px, transparent 2px),' +
  'linear-gradient(155deg, #ece0c2 0%, #e3d5b2 55%, #ddcca6 100%)';

function ForestBand() {
  return (
    <div style={{
      position: 'absolute', left: 0, top: 0, bottom: 0, width: '9.5%',
      background: FOREST_BG, backgroundSize: FOREST_BG_SIZE,
      boxShadow: 'inset -14px 0 18px -10px rgba(20,40,15,0.55)',
      pointerEvents: 'none', zIndex: 1
    }}>
      {/* soft shadow the forest casts onto the homestead ground */}
      <div style={{
        position: 'absolute', right: -22, top: 0, bottom: 0, width: 22,
        background: 'linear-gradient(90deg, rgba(40,55,30,0.18), transparent)'
      }} />
      <EdgeTag text="Vest · Pădure" />
    </div>
  );
}
...
function RiverBand() {
  return (
    <div style={{
      position: 'absolute', right: 0, top: 0, bottom: 0, width: '10.5%',
      background: 'linear-gradient(90deg, #8fc0d8 0%, #5b9ec2 30%, #3f86ae 62%, #5aa1c6 100%)',
      boxShadow: 'inset 16px 0 20px -12px rgba(20,60,80,0.5)',
      ...
      <svg viewBox="0 0 40 200" preserveAspectRatio="none"
        ...
        {[8, 17, 26, 34].map((x, i) => (
          <path key={i}
            d={`M ${x} -5 C ${x + 5} 30, ${x - 5} 60, ${x + 4} 95 S ${x - 4} 160, ${x + 3} 210`}
            fill="none" stroke="rgba(255,255,255,0.32)" strokeWidth={i % 2 ? 0.7 : 1.1}
            strokeLinecap="round" />
        ))}
      </svg>
      {/* pebbled bank on the homestead side */}
      <div style={{
        position: 'absolute', left: 0, top: 0, bottom: 0, width: 13,
        background:
          'radial-gradient(circle at 50% 20%, #b8b0a0 0 2px, transparent 3px),' +
          'radial-gradient(circle at 30% 55%, #a39a88 0 2.4px, transparent 3px),' +
          'radial-gradient(circle at 70% 80%, #c4bcac 0 2px, transparent 3px)',
        backgroundSize: '13px 16px, 13px 19px, 13px 15px',
        opacity: 0.9
      }} />
      <EdgeTag text="Est · Râu" tone="water" />
    </div>
  );
}

// Vertical poster-style edge label
function EdgeTag({ text, tone = 'forest' }) {
  return (
    <div style={{
      position: 'absolute', top: '50%', left: '50%',
      transform: 'translate(-50%, -50%) rotate(-90deg)',
      transformOrigin: 'center',
      background: tone === 'water' ? 'rgba(247,252,255,0.92)' : 'rgba(247,236,208,0.94)',
      border: `1px solid ${tone === 'water' ? 'rgba(40,90,120,0.5)' : 'rgba(110,75,30,0.5)'}`,
      color: tone === 'water' ? '#1d4a63' : '#3a2810',
      padding: '3px 12px', borderRadius: 2,
      fontFamily: 'Newsreader, Georgia, serif', fontSize: 11, fontWeight: 600,
      letterSpacing: '0.18em', textTransform: 'uppercase', whiteSpace: 'nowrap',
      boxShadow: '0 2px 5px rgba(40,40,20,0.18)'
    }}>
      {text}
    </div>
  );
...
function CompassRose() {
  return (
    <div style={{
      position: 'absolute', right: '1.2%', top: '2.5%',
      width: 46, height: 46, zIndex: 6, pointerEvents: 'none'
    }}>
      <svg viewBox="0 0 48 48" style={{
        width: '100%', height: '100%',
        filter: 'drop-shadow(0 2px 4px rgba(40,40,20,0.25))'
      }}>
        <circle cx="24" cy="24" r="20" fill="rgba(251,247,234,0.92)" stroke="rgba(110,75,30,0.5)" strokeWidth="1" />
        <polygon points="24,6 28,25 24,21 20,25" fill="#b8462f" />
        <polygon points="24,42 20,23 24,27 28,23" fill="#6b6356" />
        <text x="24" y="15.5" textAnchor="middle"
          fontFamily="Newsreader, Georgia, serif" fontSize="9" fontWeight="700" fill="#3a2810">N</text>
      </svg>
    </div>
  );
}
...
function GeneralMap({ onOpenZone, faded }) {
  return (
    <div style={{
      ...
      background:
        'radial-gradient(circle at 50% 40%, #f3ecd8 0%, #eadfc4 70%, #e3d6b6 100%)'
    }}>
      {/* Surroundings */}
      <ForestBand />
      <RiverBand />
      ...
      {/* The fenced homestead plot — all zone layers live inside, inset so the
          forest (west) and river (east) frame it like the reference plan. */}
      <div style={{ position: 'absolute', left: '12.5%', right: '13.5%', top: '8%', bottom: '7%' }}>
        {/* Leafy hedge border */}
        <div style={{
          position: 'absolute', inset: -9, borderRadius: 11,
          background: FOREST_BG, backgroundSize: FOREST_BG_SIZE,
          boxShadow: '0 6px 20px rgba(50,40,15,0.2), inset 0 0 0 1.5px rgba(35,55,25,0.6)'
        }} />
        {/* Homestead ground (clips decoration to the plot) */}
        <div style={{
          position: 'absolute', inset: 0, borderRadius: 4, overflow: 'hidden',
          background: PLOT_GROUND,
          boxShadow: 'inset 0 0 0 1px rgba(120,85,40,0.25), inset 0 2px 10px rgba(120,90,50,0.12)'
        }}>
          {/* Watercolour washes (under everything) */}
          <svg style={{
          ...
          {/* Iconic decoration (clipped to plot) */}
          <div style={{
            position: 'absolute', inset: 0, opacity: faded ? 0.4 : 1,
            pointerEvents: 'none'
          }}>
            {GENERAL_ZONES.map(z => (
        ...
        {/* Zone tiles + labels — unclipped sibling so edge labels aren't cut by the hedge */}
...
function PondCallout({ p, lx, ly }) {
  return (
    <div style={{
      ...
      <div style={{
        fontSize: 9.5, color: '#5a4528', fontStyle: 'italic',
...
const DENSITY_GRID = {
  wide: { plantsPerM2: 1.6 },
  medium: { plantsPerM2: 3.0 },
  tight: { plantsPerM2: 5.5 },
};

// Warm chocolate-brown tilled soil with speckle + clod texture (not black).
const SOIL_BG =
  'radial-gradient(circle at 18% 22%, rgba(255,220,170,0.10) 0 1.5px, transparent 2px),' +
  'radial-gradient(circle at 62% 41%, rgba(0,0,0,0.30) 0 2px, transparent 3px),' +
  'radial-gradient(circle at 83% 74%, rgba(255,220,170,0.08) 0 1.5px, transparent 2px),' +
  'radial-gradient(circle at 38% 81%, rgba(0,0,0,0.28) 0 2px, transparent 3px),' +
  'radial-gradient(ellipse at 35% 28%, #7a4f2c 0%, #5e3a1f 48%, #46290f 100%)';
const SOIL_BG_SIZE = '13px 13px, 17px 17px, 15px 15px, 19px 19px, 100% 100%';

// Deterministic PRNG so every render of a cell looks identical.
...
// Scatter `count` plant sprites across a w×h px region. Jittered grid so they
// pack densely and overlap into a lush mass; varied scale + rotation; returned
// already depth-sorted (back→front) so lower sprites paint on top.
function scatterPlants({ count, w, h, base, seed, icon, x0 = 0 }) {
  const rng = mulberry32(seed * 2654435761 >>> 0);
  const aspect = w / Math.max(0.1, h);
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      const jx = (rng() - 0.5) * dx * 0.85;
      const jy = (rng() - 0.5) * dy * 0.85;
      const scale = base * cell * (0.78 + rng() * 0.62);
      out.push({
        x: x0 + dx * (c + 0.5) + jx,
        y: dy * (r + 0.5) + jy,
        size: scale,
        rot: (rng() - 0.5) * 34,
        icon: typeof icon === 'function' ? icon(rng, x0 + dx * (c + 0.5)) : icon,
        z: rng(),
      });
    }
  }
  out.sort((a, b) => (a.y + a.size * 0.5) - (b.y + b.size * 0.5));
  return out;
}
  ...
  return (
    <img src={p.icon} alt=""
      style={{
        ...
        transform: `rotate(${p.rot}deg)`, transformOrigin: '50% 60%',
        pointerEvents: 'none',
...
const VEG_BEDS = [
  {
    ...
    cells: [
      {
        pair: {
          a: 'Strawberry', iconA: 'assets/veg/strawberry.svg', colorA: '#f08a70',
      ...
      {
        pair: {
          a: 'Garlic', iconA: 'assets/veg/garlic.svg', colorA: '#e8d8b0',
      ...
      {
        pair: {
          a: 'Raspberry', iconA: 'assets/veg/raspberry.svg', colorA: '#d870a0',
      ...
      {
        pair: {
          a: 'Blueberry', iconA: 'assets/veg/blueberry.svg', colorA: '#7eb0d4',
          b: 'Chamomile', iconB: 'assets/veg/chamomile.svg', colorB: '#dde4b0'
        },
        density: 'medium'
      },
      {
        pair: {
          a: 'Corn', iconA: 'assets/veg/corn.svg', colorA: '#f5d65a',
      ...
      {
        pair: {
          a: 'Rose', iconA: 'assets/veg/rose.svg', colorA: '#e890a8',
          b: 'Garlic', iconB: 'assets/veg/garlic.svg', colorB: '#e8d8b0'
        },
        density: 'medium'
      },
      {
        pair: {
          a: 'Apple', iconA: 'assets/veg/apple.svg', colorA: '#e8806a',
          b: 'Calendula', iconB: 'assets/veg/calendula.svg', colorB: '#f0c45a'
        },
        density: 'wide'
      },
      {
        pair: {
          a: 'Plum', iconA: 'assets/veg/plum.svg', colorA: '#a07cb8',
          b: 'Chamomile', iconB: 'assets/veg/chamomile.svg', colorB: '#dde4b0'
        },
        density: 'wide'
      },
...
function CompanionPill({ a, b, colorA, colorB, fontSize, maxWidth }) {
  ...
  return (
    <div style={{
      ...
      <span style={{
        padding: pad, background: colorA, color: '#231a08',
        textShadow: '0 1px 0 rgba(255,255,255,0.35)'
      }}>{aT}</span>
      <span style={{
        padding: `${Math.max(1.5, fontSize * 0.22)}px ${Math.max(2, fontSize * 0.25)}px`,
        background: '#2a1a08', color: '#f5e6c4',
        display: 'flex', alignItems: 'center', fontWeight: 600
      }}>+</span>
      <span style={{
        padding: pad, background: colorB, color: '#231a08',
        textShadow: '0 1px 0 rgba(255,255,255,0.35)'
      }}>{bT}</span>
    </div>
  );
...
function CompanionCell({ cell, cellWM, cellHM, pxPerM, pxPerMY, seed }) {
  ...
  const per = Math.min(46, Math.max(5, Math.round(halfArea * DENSITY_GRID[cell.density].plantsPerM2)));
  const halfWPx = cellWPx / 2;
  const base = cell.density === 'wide' ? 1.55 : cell.density === 'medium' ? 1.3 : 1.1;
  const left = scatterPlants({ count: per, w: halfWPx, h: cellHPx, base, seed: seed * 7 + 1, icon: p.iconA, x0: 0 });
  const right = scatterPlants({ count: per, w: halfWPx, h: cellHPx, base, seed: seed * 7 + 53, icon: p.iconB, x0: halfWPx });
  const all = [...left, ...right].sort((a, b) => (a.y + a.size * 0.5) - (b.y + b.size * 0.5));
  const pillFont = Math.max(6.5, Math.min(11, cellWPx * 0.075, cellHPx * 0.16));
  const pillMaxW = Math.max(40, cellWPx - 4);
  return (
    <div style={{
      position: 'relative', width: '100%', height: '100%',
      background: SOIL_BG, backgroundSize: SOIL_BG_SIZE,
      boxShadow: 'inset 0 2px 7px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(30,15,4,0.4)',
      overflow: 'hidden'
    }}>
      {all.map((t, i) => <PlantSprite key={i} p={t} />)}
      <div style={{
        position: 'absolute', top: '50%', left: '50%',
...
function BedCell({ cell, cellWM, cellHM, pxPerM, pxPerMY, seed = 0 }) {
  ...
  const count = Math.min(95, Math.max(5, Math.round(area * DENSITY_GRID[cell.density].plantsPerM2)));
  const base = cell.density === 'wide' ? 1.55 : cell.density === 'medium' ? 1.3 : 1.1;
  const sprites = scatterPlants({ count, w: cellWPx, h: cellHPx, base, seed: seed * 13 + 5, icon: cell.icon });
  ...
  return (
    <div style={{
      ...
      {sprites.map((p, i) => <PlantSprite key={i} p={p} />)}
      <div style={{
        position: 'absolute', top: '50%', left: '50%',
        transform: 'translate(-50%, -50%)', zIndex: 2
      }}>
...
function GardenCanvas({ activeZone = 'general', onOpenZone,
  ...
  return (
    <div style={{
      ...
      {/* Top ruler (hidden on the framed General plan) */}
      {!isGeneral && (
        <div style={{ display: 'flex', flexShrink: 0 }}>
          <div style={{
            width: 22, height: 22, background: 'var(--ruler)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        <div ref={wrapRef} style={{
          ...
          {/* Compass (detail views only — General has its own framed compass) */}
          {!isGeneral && (
            <div style={{
